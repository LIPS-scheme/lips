 ...
 *           auto formatting (when copy paste single line)
 *
...
function terminal({selector, lips, dynamic = false, name = 'terminal'}, undefined) {
            ...
            if (e.text) {
                ...
                try {
                    ...
                    if (!code.match(/\n/)) {
                        ...
                        var space = new Array(prompt.length + 1).join(' ');
                        output = code.split('\n').map(function(line, i) {
                            return (i === 0 ? '' : space) + line;
                        }).join('\n');
                    }
                } catch(e) {
                    console.log(e);
    ...
    term.on('mouseover', '.token.function, .token.keyword, .token.operator, .token.builtin', function() {
           var self = jQuery(this);
           var lips = term.interpreter;
           var fn = lips.__env__.get(self.data('text'), { throwError: false });
           var node = self;
           var left;
           var cls = self.attr('class').split(' ').find(function(cls) {
               return cls !== 'token';
           });
           if (self.closest('.cmd') && self.prev().is('.' + cls)) {
               node = self.prevUntil(':not(.' + cls + ')').last();
           }
           if (fn && fn.__doc__) {
               self.addClass('__doc__');
               self.attr('data-doc', fn.__doc__.replace(/\n\n/g, '\n \n'));
               self.css('--left', node.position().left);
           }
       }).on('mouseout', '.token.__doc__', function() {
          var self = jQuery(this).removeClass('__doc__');
