javascript:(function(next) {
    ...
    async function hash() {
        try {
            var res = await fetch('https://api.github.com/repos/jcubic/lips/commits?sha=master');
    ...
    function init(boostrap) {
        ...
        var term = terminal({ selector: $('<div class="lips">').appendTo('body'), name: 'lips', lips });
        if (boostrap) {
