(test.skip "macroexpand: should not expand let bindings"
      (lambda (t)
        (t.snapshot (macroexpand (let ((++ (lambda (a b) (* a b)))) (++ 1 2))))
        (t.snapshot (macroexpand (let ((++ 10)) (+ ++ ++))))))

(test.skip "macroexpand: should only expand macro in body of let"
      (lambda (t)
...
(test.skip "macroexpand: should not expand lambda parameters"
      (lambda (t)
        (define-macro (foo x) `(1 2 3 ,x))
        (define-macro (bar x) `(x y z ,x))
...
(test.skip "macroexpand: should not expand shadowed procedure name in recursive call"
      (lambda (t)
        (define-macro (foo x) `(1 2 3 ,x))
        (define-macro (bar x) `(x y z ,x))
...
(test.skip "macroexpand: should expand macro inside letrec function"
      (lambda (t)
        (define-macro (foo x) `(1 2 3 ,x))
        (define-macro (bar x) `(x y z ,x))
