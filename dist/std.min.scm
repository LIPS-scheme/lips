(define (%doc string fn) (typecheck "%doc" fn "function") (typecheck "%doc" string "string") (set-obj! fn (quote __doc__) (--> string (replace #/^ +/gm ""))) fn)(define-macro (let-syntax vars . body) "(let-syntax ((name fn)) body)
...
|     | |    |   / /\\ \\   |  | |      |  |     | |    | |      |   |  |
+     |_|    +  /_/  \\_\\  +  |_|      +  +     |_|    |_|      +   +  +
 \\_           \\_           \\_       _/    \\_                 _/  _/ _/" ((lambda (x) (x x)) (lambda (g) (h (lambda args (apply (g g) args)))))))(define string-append concat)(define = ==)(define remainder %)(define -inf.0 Number.NEGATIVE_INFINITY)(define +inf.0 Number.POSITIVE_INFINITY)(define procedure? function?)(define expt **)(define list->vector list->array)(define vector->list array->list)(define-macro (define-symbol-macro type spec . rest) "(define-symbol-macro type (name . args) . body)
...
need to have same type" (if (string=? (type a) (type b)) (cond ((number? a) (= a b)) ((pair? a) (and (null? a) (null? b))) (else (eq? a b))) #f))(define (equal? a b) "(equal? a b)
...
It return false if port is closed." (let ((port (if (null? rest) (current-input-port) (car rest)))) (typecheck "char-ready?" port "input-port") (port.char_ready)))(if (eq? global self) (set! self.fs (require "fs")))(define open-input-file (let ((readFile #f)) (lambda (filename) "(open-input-file filename)

Function return new Input Port with given filename. In Browser user need to
provide global fs variable that is instance of FS interface." (if (null? self.fs) (throw (new Error "open-input-file: fs not defined")) (begin (if (not (procedure? readFile)) (let ((_readFile (promisify fs.readFile))) (set! readFile (lambda (filename) "(readFile filename)
